...
from multiclassification.celery_app import celery_app
...
def _phase1_analyze(job_id: str, state: dict):
    from multiclassification.agents.problem_analyzer import problem_analyzer_node
    state = _run_async(problem_analyzer_node(state))
    _save(job_id, state)


def _phase2_profile_and_run(job_id: str, state: dict):
    from multiclassification.agents.data_profiler import data_profiler_node
    from multiclassification.agents.pipeline_designer import pipeline_designer_node
    from multiclassification.graph import parallel_experiments_node
    from multiclassification.agents.model_selector import model_selector_node
...
def _phase3_output(job_id: str, state: dict):
    from multiclassification.agents.documentation_agent import documentation_agent_node
    from multiclassification.agents.output_agent import output_agent_node
...
@celery_app.task(name="multiclassification.tasks.run_pipeline_task")
...
@celery_app.task(name="multiclassification.tasks.resume_pipeline_task")
