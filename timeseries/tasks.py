...
from timeseries.celery_app import celery_app
from timeseries.db import get_job, save_job
...
def _phase1_analyze(job_id: str, state: dict):
    from timeseries.agents.problem_analyzer import problem_analyzer_node
    state = _run_async(problem_analyzer_node(state))
    _save(job_id, state)


def _phase2_profile_and_run(job_id: str, state: dict):
    from timeseries.agents.data_profiler import data_profiler_node
    from timeseries.agents.pipeline_designer import pipeline_designer_node
    from timeseries.graph import parallel_experiments_node
    from timeseries.agents.model_selector import model_selector_node
...
def _phase3_output(job_id: str, state: dict):
    from timeseries.agents.documentation_agent import documentation_agent_node
    from timeseries.agents.output_agent import output_agent_node
...
@celery_app.task(name="timeseries.tasks.run_pipeline_task")
...
@celery_app.task(name="timeseries.tasks.resume_pipeline_task")
def resume_pipeline_task(job_id: str, resume_at: str, payload: Dict[str, Any]):
    ...
    try:
        if resume_at == "await_problem_approval":
            ...
            if corrections.get("date_column"):
                state["date_column"] = corrections["date_column"]
            if corrections.get("target_column"):
                state["target_column"] = corrections["target_column"]
            if corrections.get("forecast_horizon"):
                try:
                    state["forecast_horizon"] = int(corrections["forecast_horizon"])
                except (ValueError, TypeError):
                    pass
            if corrections.get("evaluation_metric"):
                state["evaluation_metric"] = corrections["evaluation_metric"]
            _save(job_id, state)
            ...
            if selected_id:
                for exp in state.get("experiment_results", []):
                    if exp.get("experiment_id") == selected_id:
                        ...
                        state["forecast_values"] = exp.get("forecast_values")
                        break
            _save(job_id, state)
