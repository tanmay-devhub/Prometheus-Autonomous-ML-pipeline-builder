...
from timeseries.config import MLFLOW_TRACKING_URI
...
class MLflowTracker:
    ...
    def start_run(self, job_id: str, architecture_name: str) -> str:
        experiment_name = f"prometheus_ts_{job_id}"
        try:
    ...
    def get_best_run(self, job_id: str, metric: str) -> Optional[Dict]:
        ...
        runs = mlflow.search_runs(
            ...
            order_by=[f"metrics.{metric} ASC"],  # lower is better for RMSE/MAE/MAPE
            max_results=1,
        )
        if runs.empty:
            return None
