...
from timeseries.state import TimeSeriesState, ExperimentResult
from timeseries.agents.code_generator import code_generator_node
from timeseries.agents.result_interpreter import result_interpreter_node
from timeseries.agents.failure_diagnostician import failure_diagnostician_node
from timeseries.agents.fix_executor import fix_executor_node
from timeseries.config import MAX_RETRIES
from shared.execution.e2b_executor import E2BExecutor
from timeseries.tracking.mlflow_tracker import MLflowTracker


async def _run_single_experiment(
    state: TimeSeriesState, architecture: Dict, experiment_id: str
) -> ExperimentResult:
    ...
    # Log metrics (lower RMSE = better, unlike classification)
    rmse_val = experiment_result["parsed_metrics"].get("rmse", float("inf"))
    mape_val = experiment_result["parsed_metrics"].get("mape", 0.0)
    tracker.log_metrics(run_id, {
        "rmse": float(rmse_val) if rmse_val != float("inf") else 9999.0,
        "mape": float(mape_val),
    })
...
async def parallel_experiments_node(state: TimeSeriesState) -> TimeSeriesState:
