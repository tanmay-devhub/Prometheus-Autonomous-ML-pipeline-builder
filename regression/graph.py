...
from regression.agents.code_generator import code_generator_node
from regression.agents.result_interpreter import result_interpreter_node
from regression.agents.failure_diagnostician import failure_diagnostician_node
from regression.agents.fix_executor import fix_executor_node
from regression.config import MAX_RETRIES
from shared.execution.e2b_executor import E2BExecutor
from regression.tracking.mlflow_tracker import MLflowTracker


async def _run_single_experiment(
    state: RegressionState, architecture: Dict, experiment_id: str
) -> ExperimentResult:
    ...
    tracker.log_metrics(run_id, {
        metric: float(metric_val) if metric_val == metric_val else 0.0,
        **{k: float(v) for k, v in experiment_result["parsed_metrics"].items()
           if k in ("rmse", "mae", "r2") and k != metric},
    })
...
async def parallel_experiments_node(state: RegressionState) -> RegressionState:
