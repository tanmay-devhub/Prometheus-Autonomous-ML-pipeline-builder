...
from regression.config import MLFLOW_TRACKING_URI
...
class MLflowTracker:
    ...
    def start_run(self, job_id: str, architecture_name: str) -> str:
        experiment_name = f"regression_{job_id}"
        try:
            experiment_id = mlflow.create_experiment(experiment_name)
        except mlflow.exceptions.MlflowException:
            experiment_id = mlflow.get_experiment_by_name(experiment_name).experiment_id

        run = mlflow.start_run(experiment_id=experiment_id, run_name=architecture_name)
        mlflow.end_run()
