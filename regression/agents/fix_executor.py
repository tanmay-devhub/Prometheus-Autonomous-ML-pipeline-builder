...
FIX_PROMPT_TEMPLATE = """You are fixing a broken ML regression pipeline script.
...
async def fix_executor_node(
    ...
    if should_regenerate:
        state["debug_log"].append({
            ...
            "reason": f"failure_type '{current_failure}' repeated",
            "experiment_id": experiment_result.get("experiment_id"),
        ...
        fixed_code = await router.call(
            task_type="code_fix",
            system_prompt="You are an expert Python ML engineer fixing a broken regression pipeline script.",
